...
const uploadBarber = createUploader('barbers');
...
router.put('/:id/schedule',
  ...
  [
    body('schedules').isArray().withMessage('schedules debe ser un arreglo'),
    body('schedules.*.day_of_week').isInt({ min: 1, max: 7 }).withMessage('day_of_week debe ser entre 1 y 7'),
    body('schedules.*.start_time').notEmpty().withMessage('start_time es obligatorio'),
    body('schedules.*.end_time').notEmpty().withMessage('end_time es obligatorio'),
  ],
